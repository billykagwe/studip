  ...
  const handleClick = () => {
    ...
    return (
        <tr className='hover:shadow-md cursor-pointer' onClick={handleClick}>
          ...
          <input id={name} className='mr-6 block w-4 h-4' checked={checked} name="plan" type="checkbox"/>
            </td>
          ...
          {company ? <td className={`px-4 text-gray-600 py-3  text-md hover:text-teal-400`}>
              <Link  href={`/company-profile/${company_id}`}>
                  <a >{company}</a> 
                </Link>
          </td> : <td className={`px-4 text-gray-600 py-3  text-md`}>
              {attached ?  <Link href={`/company-profile/${company_id}`}>
                  Attached
                </Link> : 'Not Yet' } 
             </td> }
