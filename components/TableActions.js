    ...
    return ( 
        <>
         ...
         <div className='mx-2'>
            ...
            <select value={filters.school} onChange={(e) => setFilters({...filters,school:e.target.value})} className="form-select w-full p-2 rounded text-gray-700 bg-gray-100">
              <option >School</option>
            {data?.schools?.map(school => (<option key={school?.id}  value={school?.name}>{school?.name}</option>))}
            </select>
        </div>
       <div className='mx-2'>
         
            <select value={filters.course} onChange={(e) => setFilters({...filters,course:e.target.value})} className="form-select inline w-full p-2 rounded text-gray-700 bg-gray-100">
